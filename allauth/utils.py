...
def _generate_unique_username_base(txts, regex=None):
    ...
    for txt in txts:
        ...
        # Django allows for '@' in usernames in orders to accomodate for
...
def set_form_field_order(form, field_order):
    ...
    field_order is a list of field names specifying the orders. Append fields
    not included in the list in the default orders for backward compatibility
    with subclasses not overriding field_order. If field_order is None, keep
    all fields in the orders defined in the class. Ignore unknown fields in
    ...
    for key in field_order:
        try:
            ...
            pass
    fields.update(form.fields)  # add remaining fields in original orders
    form.fields = fields
